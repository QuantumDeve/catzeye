$(document).ready(function () {
    ...
    $(document).on("submit",'#submit-event',function(e){
        e.preventDefault();
        var form_data = new FormData($('#submit-event')[0]);
        $.ajax({
            headers: {
            ...
            success: function (result) {
                if (result.status) {
                    ...
                    $.each(result.message, function (key, value) {
                        if (value[0])
                            $("#submit-event .error_" + key).html(value[0]);
                    });
                }
    ...
    $("body").on("click",".add-more",function(){ 
        ...
        var html = '<input type="file" class="form-control" id="image" name="image[]"> <div class="val-error error_image[]" style="color:red">';
        //  $(html).find(".change").prepend("<label for=''>&nbsp;</label><br/><a class='btn btn-danger remove'>- Remove</a>");
      
          $(html).find(".change").html("<label for=''>&nbsp;</label><br/><a class='btn btn-danger remove'>- Remove</a>");
        //   $('file[name="image[]"]:last').remove();
        //   $(this).closest('div').find('file[name="image"]:last').remove();
